...
function CardPage() {
  ...
  const [total, setTotal] = useState(
    { brand: "Apple",
      ...
      rating: 4.69,
      stock: 94,
      thumbnail: "https://content2.onliner.by/catalog/device/main/74080a1b5a15d0ffe0a4ac9ad3a8279b.jpeg",
      title: "iPhone 9",
  ...
  useEffect(() => {
    async function componentDidMount() {
      try {
        const result = await axios.get(`https://63a042fa24d74f9fe832fb1e.mockapi.io/items?id=${id}`);
  ...
  <div>
    <div className={style.breadcrumbs}> 
      <div>
        <a href="/" className={style.link}> <span >STORE </span> </a> 
      </div>
    ...
    <div className={style.cardpage__container} > 
        ...
        <div className={style.info__container}>
            ...
            <img src={total.thumbnail} style={{width: "35vh" , height: "35vh" , borderRadius: "20px"}} alt=""></img>
            </div>
            <div className={style.product_info}>
                <div className={style.container__font}>Category: {total.category} </div>
                <div className={style.container__font}>Brand: {total.brand}</div>
                <div className={style.container__font}>Description: {total.description}</div>
                <div className={style.container__font}>Rating: {total.rating}</div>
                <div className={style.container__font}>Stock: {total.stock}</div>
                <div className={style.container__font}>Discount Percentage: {total.discountPercentage}</div>
            </div>
            <div>
